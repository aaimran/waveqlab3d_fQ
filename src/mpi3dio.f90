module mpi3dio
  ...
  end interface write_file_distributed
  ...
  end interface read_file_distributed
...
contains
  ...
  subroutine init_io(C)
    ...
    type(cartesian3d_t),intent(inout) :: C
    ...
    call subarray(C%nq,C%nr,C%ns,C%mq,C%pq,C%mr,C%pr,C%ms,C%ps,MPI_REAL_PW,C%array_w)
    call subarray(C%nq,C%nr,C%ns,C%mq,C%pq,C%mr,C%pr,C%ms,C%ps,MPI_REAL_PS,C%array_s)
